...
//                    << "handling event!:  " << registeredParent.path.str() << " changed path: " << diff.first.str();
    ...
    void DatastoreTransaction::splitToMany(Path p, dynamic input, vector<std::pair<string, dynamic>> &result) {
...
//            MLOG(MINFO) << "size: " << result.size() <<  " idem spracovat " << item.first.c_str() << " path: " << currentPath;
    ...
    map<Path, DatastoreDiff> DatastoreTransaction::splitDiff(DatastoreDiff diff) {
        ...
        if (diff.type == DatastoreDiffType::create) {
            //MLOG(MINFO) << "dosiel mi " << toPrettyJson(diff.after);
            splitToMany(diff.path, diff.after, split);
            for (const auto &s : split) {
                diffs.emplace(s.first, DatastoreDiff(diff.before, s.second, diff.type, Path(s.first)));
                //MLOG(MINFO) << s.first << " json : " << toPrettyJson(s.second);
            }
            return diffs;
        } else if (diff.type == DatastoreDiffType::deleted) {
            splitToMany(diff.path, diff.before, split);
            for (const auto &s : split) {
                diffs.emplace(s.first, DatastoreDiff(s.second, diff.after, diff.type, Path(s.first)));
                //MLOG(MINFO) << s.first << " json : " << toPrettyJson(s.second);
            }
            return diffs;
        }
